...
export default function Whiteboard({ roomId, ywsUrl }: WhiteboardProps) {
  ...
  useEffect(() => {
    ...
    // YWS ist auf demselben Server wie der API-Server (Port 3001) oder separate App (Port 5179)
    const ywsPort = (window as any).YWS_PORT || import.meta.env.VITE_YWS_PORT || 3001;
    const hostname = new URL(ywsUrl || window.location.href).hostname;
    const url = ywsUrl || `${proto}://${hostname}:${ywsPort}/yws?room=${roomId}`;
